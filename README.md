...
- `match_group`: (optional) String or array of strings containing groups IDs. The group will get affinity scores with respect to the submitted papers based on their expertise. This expertise is obtained based on the publications available in OpenReview. If 2 group IDs are given, the members of the two groups will be matched based on their expertise. It can be provided instead of or on top of the `reviewer_ids` and `csv_expertise`, the values will be combined.
