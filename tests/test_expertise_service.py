...
class TestExpertiseService:
    ...
    def openreview_context(self):
        ...
        app = expertise.service.create_app(config=config)

        with app.app_context():
            yield {"app": app, "test_client": app.test_client(), "config": config}

    def test_request_expertise_with_no_config(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        test_client = openreview_context["test_client"]
        # Submitting an empty config with no required fields
        response = test_client.post(
            "/expertise", data=json.dumps({}), content_type="application/json"
        )
        assert response.status_code == 400, f"{response.json}"
        assert "bad request" in response.json["error"].lower()
        assert (
            response.json["error"]
            == "Bad request: missing required field: name match_group paper_invitation"
        )

    def test_request_expertise_with_missing_required_fields(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        ...
        assert (
            response.json["error"]
            == "Bad request: missing required field: paper_invitation"
        )

    def test_request_expertise_with_invalid_field(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        ...
        assert "bad request" in response.json["error"].lower()
        assert (
            response.json["error"] == "Bad request: unexpected field: unexpected_field"
        )

    def test_request_expertise_with_invalid_and_missing_required_field(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        # Submit a working config with an extra field that is not allowed
        test_client = openreview_context["test_client"]
    ...
    def test_request_expertise_with_invalid_model_param(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        ...
        assert "bad request" in response.json["error"].lower()
        assert (
            response.json["error"] == "Bad request: unexpected model param: dummy_param"
        )

    def test_request_expertise_with_valid_parameters(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        ...
        response = test_client.get(
            "/expertise/status", query_string={"id": f"{job_id}"}
        ).json["results"]
        ...
        assert (
            ...
            == "Job is complete and the computed scores are ready"
        )
    ...
    def test_get_results_by_job_id(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        test_client = openreview_context["test_client"]
        ...
        for item in response:
            submission_id, profile_id, score = (
                item["submission"],
                item["user"],
                float(item["score"]),
            )
    ...
    def test_get_results_for_all_jobs(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        # Assert that there are two completed jobs belonging to this user
        test_client = openreview_context["test_client"]
        response = test_client.get("/expertise/status/all", query_string={}).json[
            "results"
        ]
        assert len(response) == 1
    ...
    def test_get_results_and_delete_data(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        ...
        response = test_client.get(
            "/expertise/results",
            query_string={
                "id": f"{openreview_context['job_id']}",
                "delete_on_get": True,
            },
        ).json["results"]
    ...
    def test_request_expertise_with_model_errors(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
    ...
    def test_get_results_and_get_error(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        MAX_TIMEOUT = 600  # Timeout after 10 minutes
        assert openreview_context["job_id"] is not None
        ...
        while response[0]["status"] == "Processing" and try_time <= MAX_TIMEOUT:
            ...
            response = test_client.get(
                "/expertise/status",
                query_string={"id": f"{openreview_context['job_id']}"},
            ).json["results"]
            try_time = time.time() - start_time
        ...
        assert (
            response[0]["description"]
            == "use_title and use_abstract cannot both be False"
        )
    ...
    def test_fetch_high_load_results(
        self, openreview_context, celery_session_app, celery_session_worker
    ):
        ...
        response = test_client.get(
            "/expertise/status", query_string={"id": f"{last_job_id}"}
        ).json["results"]
        assert len(response) == 1
        start_time = time.time()
        try_time = time.time() - start_time
        ...
        for id in id_list:
            ...
            response = test_client.get(
                "/expertise/status", query_string={"id": f"{id}"}
            ).json["results"]
            assert response[0]["status"] == "Completed"
            assert response[0]["name"] == "test_run"
            assert (
            ...
            response = test_client.get(
                "/expertise/results",
                query_string={"id": f"{id}", "delete_on_get": True},
            )
            metadata = response.json["metadata"]
            assert metadata["submission_count"] == 2
            response = response.json["results"]
