...
def test_elmo_queue(openreview_context, celery_app, celery_worker):
    ...
    csv_list = []
    with open(os.path.join('tests/data/', 'csv_submissions.csv'), 'r') as f:
        csv_list = f.readlines()
    config['csv_submissions'] = csv_list
    shutil.copytree('tests/data/archives', f"./{server_config['WORKING_DIR']}/~{test_num}/0/archives") # This will get generated by a real API call
    ...
    assert not os.path.isfile(f"{server_config['WORKING_DIR']}/~{test_num}/0/test_run.csv")
    ...
    with open(os.path.join('tests/data/', 'csv_submissions.csv'), 'r') as f:
        csv_list = f.readlines()
    config['csv_submissions'] = csv_list
    shutil.copytree('tests/data/archives', f"./{server_config['WORKING_DIR']}/~{test_num}/1/archives") # This will get generated by a real API call
    response = test_client.post(
        '/expertise',
        data = json.dumps({'TEST_NUM': test_num, **config}),
        content_type='application/json'
