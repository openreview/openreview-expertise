...
def ranking(
    archives_dataset, submissions_dataset, publication_id_to_profile_id, worker
):
    for note_id, submission in tqdm(
        submissions_dataset.items(), total=len(submissions_dataset), position=worker
    ):
        removed_publication = None
        for profile_id in publication_id_to_profile_id[note_id]:
            removed_publication = (
                archives_dataset.remove_publication(note_id, profile_id)
                or removed_publication
            )
        bm25Model = bm25.Model(
            archives_dataset,
            submissions_dataset,
            use_title=config["model_params"]["use_title"],
            use_abstract=config["model_params"]["use_abstract"],
        )
        reviewer_scores = bm25Model.score(submission)

        sorted_profile_ids = [
            (profile_id, value)
            for profile_id, value in sorted(
                reviewer_scores.items(), key=lambda item: item[1], reverse=True
            )
        ]
...
def evaluate_scores(scores_path, publication_id_to_profile_id, rank):
    within_rank = 0
    total = 0
    for submission_file in tqdm(
        scores_path.iterdir(), total=len(list(scores_path.glob("*")))
    ):
        total += 1
        ...
        with open(submission_file, "rb") as file_handle:
            ...
            for idx, (sorted_profile_id, value) in enumerate(sorted_profile_ids):
                ...
                if idx >= rank:
                    break
    return "Within top {rank}: {within_rank}, Total: {total}, Ratio: {ratio}".format(
        rank=rank, within_rank=within_rank, total=total, ratio=(within_rank / total)
    )
...
if __name__ == "__main__":
    ...
    archives_dataset = ArchivesDataset(
        archives_path=Path(config["dataset"]["directory"]).joinpath("archives")
    )
    ...
    if config["model"] == "bm25":
        ...
        for idx, (profile_id, publications) in enumerate(archives_dataset.items()):
            if idx % (len(archives_dataset) // (workers) + 1) >= len(
                archives_dataset
            ) // (workers):
            ...
            for publication in publications:
                if (
                    config["model_params"]["use_title"]
                    and "title" not in publication["content"]
                ):
                    continue
                if (
                    config["model_params"]["use_abstract"]
                    and "abstract" not in publication["content"]
                ):
                    continue
        ...
        if only_evaluate:
            print(
                evaluate_scores(
                    Path("./test"),
                    publication_id_to_profile_id,
                    config["test_params"]["rank"],
                )
            )
            sys.exit(0)
        ...
        for worker in range(workers):
            processes.append(
                multiprocessing.Process(
                    target=ranking,
                    args=(
                        copy.deepcopy(archives_dataset),
                        SubmissionsDataset(
                            submissions_dict=copy.deepcopy(submissions_dicts[worker])
                        ),
                        copy.deepcopy(publication_id_to_profile_id),
                        worker,
                    ),
                )
            )
