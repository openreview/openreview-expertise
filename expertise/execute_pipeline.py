...
def run_pipeline(api_request_str, working_dir=None):
    # Add debug output right at the beginning
    print('DEBUG: Starting run_pipeline')
    print(f'DEBUG: Type of input: {type(api_request_str)}')
    print(f'DEBUG: Length of input: {len(api_request_str) if isinstance(api_request_str, str) else "not a string"}')
    print(f'DEBUG: First 100 chars: {api_request_str[:100] if isinstance(api_request_str, str) else "not a string"}')
    
    try:
        # If it's a file path instead of a JSON string, try reading it
        if isinstance(api_request_str, str) and os.path.exists(api_request_str):
            print(f'DEBUG: Input appears to be a file path: {api_request_str}')
            with open(api_request_str, 'r') as f:
                api_request_str = f.read()
                print(f'DEBUG: Read file content, length: {len(api_request_str)}')
        ...
        try:
            ...
            print(f'DEBUG: Char at position {e.pos}: {api_request_str[e.pos:e.pos+10]}')
            
            # Try to fix common formatting issues
            print('DEBUG: Attempting to fix JSON formatting...')
            if api_request_str.startswith("'") and api_request_str.endswith("'"):
                api_request_str = api_request_str[1:-1].replace("\\'", "'").replace('\\"', '"')
                print('DEBUG: Removed outer quotes and escaped quotes')
            
            # Try parsing again
            try:
                raw_request: dict = json.loads(api_request_str)
                print('DEBUG: Successfully parsed JSON after fixes')
            except Exception as e2:
                print(f'DEBUG: Still failed to parse JSON: {e2}')
                raise
    
        # Log the keys in the request
        print(f'DEBUG: Request keys: {", ".join(raw_request.keys())}')
        ...
        print(f'DEBUG: Unexpected error in run_pipeline: {type(e).__name__}: {str(e)}')
        import traceback
        traceback.print_exc()
        raise
...
if __name__ == '__main__':
    ...
    # Add debug output before calling run_pipeline
    print(f'DEBUG: Input argument: {args.api_request_str[:100] if len(args.api_request_str) > 100 else args.api_request_str}')
