...
from expertise.models import centroid_scibert_cls as centroid_scibert
from expertise.train.__main__ import train_model
...
def train(config):
    ...
    optimizer = optim.Adam(
        model.parameters(), lr=config.learning_rate, weight_decay=config.l2penalty
    )
    ...
    bert_lookup = utils.load_pkl(
        os.path.join(config.kp_setup_dir, "bert_lookup_cls.pkl")
    )
    ...
    for counter, batch in enumerate(batcher.batches(batch_size=config.batch_size)):
        ...
        if counter % config.eval_every == 0:
            ...
            prediction_filename = config.train_save(
                predictions, "dev_predictions/dev.predictions.{}.jsonl".format(counter)
            )
            ...
            config.train_save(
                score_lines, "dev_scores/dev.scores.{}.tsv".format(counter)
            )
