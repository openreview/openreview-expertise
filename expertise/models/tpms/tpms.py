...
def train(config):
    print(
        "Nothing to train. This model is a shell that reads in pre-computed TPMS scores."
    )
    assert os.path.exists(
        config.tpms_scores_file
    ), "This model requires a pre-computed tpms score file."
...
def infer(config):
    print(
        "Nothing to infer. This model is a shell that reads in pre-computed TPMS scores."
    )
    assert os.path.exists(
        config.tpms_scores_file
    ), "This model requires a pre-computed tpms score file."
