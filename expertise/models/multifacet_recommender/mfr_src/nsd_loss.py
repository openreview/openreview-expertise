...
def estimate_coeff_mat_batch_max_iter(target_embeddings, basis_pred, device):
    ...
    for i in range(n_not_sparse):
        ...
        coeff_mat_trans_temp = torch.zeros(
            batch_size,
            basis_pred.size(1),
            target_embeddings.size(1),
            requires_grad=False,
...
def estimate_coeff_mat_batch_softmax(
    target_embeddings, basis_pred, device, loss_type="dist", target_norm=False
):
    batch_size = target_embeddings.size(0)
    n_basis = basis_pred.size(1)
    if n_basis == 1 and loss_type[:4] != "dist":
        coeff_mat = torch.ones(
            batch_size,
            target_embeddings.size(1),
            n_basis,
            requires_grad=False,
    ...
    if target_norm:
        ...
        target_embeddings_norm = target_embeddings / (
            0.000000000001 + target_embeddings.norm(dim=2, keepdim=True)
        )  # If this step is really slow, consider to do normalization before doing unfold
    else:
        target_embeddings_norm = target_embeddings
...
def estimate_coeff_mat_batch_max(
    target_embeddings,
    basis_pred,
    device,
    loss_type="dist",
    ...
    if loss_type[:4] == "dist":
        ...
        if always_norm_one:
            # max_v[max_v!=0] = 1
            norm_max = torch.gather(
                basis_pred_norm.expand(batch_size, n_basis, cos_sim.size(2)),
                dim=1,
                index=max_i_cos,
            )
            max_v = 1 / norm_max
...
def estimate_prod_coeff_mat_batch_opt(
    target_embeddings, basis_pred, L1_losss_B, device, coeff_opt, lr, max_iter
):
    batch_size = target_embeddings.size(0)
    ...
    if coeff_opt == "sgd":
        ...
        raise RuntimeError(
            "%s not implemented for coefficient estimation. Please check args."
            % coeff_opt
        )
...
def estimate_coeff_mat_batch_opt(
    target_embeddings, basis_pred, L1_losss_B, device, coeff_opt, lr, max_iter
):
    batch_size = target_embeddings.size(0)
    mr = MR(batch_size, target_embeddings.size(1), basis_pred.size(1), device=device)
...
def estimate_coeff_mat_batch(
    target_embeddings, basis_pred, L1_losss_B, device, max_iter=100
):
    ...
    # lr = 0.1
    for i in range(max_iter):
        coeff_mat, coeff_mat_grad = update_B_from_AC(
            basis_pred, coeff_mat_prev, target_embeddings, A, lr
        )
...
def target_emb_preparation(
    target_index,
    embeddings,
    n_batch,
    n_set,
    target_index_shuffle,
    rand_neg_method,
    target_norm,
    target_linear_layer,
):
    ...
    if target_linear_layer is not None:
        target_embeddings = torch.bmm(
            target_embeddings,
            target_linear_layer.expand(
                target_embeddings.size(0),
                target_linear_layer.size(0),
                target_linear_layer.size(1),
            ),
        )
    ...
    if rand_neg_method == "rotate":
        target_embeddings_rotate = torch.cat(
            (
                target_embeddings[target_index_shuffle:, :, :],
                target_embeddings[:target_index_shuffle, :, :],
            ),
            dim=0,
        )
        ...
        if target_linear_layer is not None:
            target_embeddings_rotate = torch.bmm(
                target_embeddings_rotate,
                target_linear_layer.expand(
                    target_embeddings.size(0),
                    target_linear_layer.size(0),
                    target_linear_layer.size(1),
        ...
        if target_norm:
            target_embeddings_rotate = target_embeddings_rotate / (
                0.000000000001 + target_embeddings_rotate.norm(dim=2, keepdim=True)
            )  # If this step is really slow, consider to do normalization before doing unfold
...
def compute_loss_set(
    ...
    rand_neg_method,
    target_norm,
    compute_div_reg=True,
    target_linear_layer=None,
    pre_avg=False,
    always_norm_one=False,
):
    ...
    if rand_neg_method == "paper_uniform":
        target_index_shuffle = torch.multinomial(
            w_freq_sampling, target_set.nelement(), replacement=True
        ).view(target_set.size())
        ...
        target_index_shuffle = torch.randint_like(
            target_set, low=1, high=w_freq.size(0), device=device
        )
    ...
    if compute_target_grad:
        target_embeddings, target_emb_neg = target_emb_preparation(
            target_set,
            entpair_embs,
            n_batch,
            n_set,
    ...
    with torch.no_grad():
        ...
        if rand_neg_method == "paper_uniform":
            target_freq_inv_norm_neg = compute_target_freq_inv_norm(
                w_freq, target_index_shuffle
            )
            neg_total_weights = target_len.sum() / float(
                target_freq_inv_norm_neg.nelement()
            )
            repeat_num_inv = 1 / repeat_num.float()
            repeat_num_inv_norm = repeat_num_inv / repeat_num_inv.mean()
            target_freq_inv_norm_neg *= (
                neg_total_weights * repeat_num_inv_norm.unsqueeze(dim=1)
            )
            ...
            target_freq_inv_norm_neg = target_freq_inv_norm.view(-1)[shuffle_idx].view(
                target_freq_inv_norm.size()
            )
        elif rand_neg_method == "rotate":
            target_freq_inv_norm_neg = torch.cat(
                (
                    target_freq_inv_norm[target_index_shuffle:, :],
                    target_freq_inv_norm[:target_index_shuffle, :],
                ),
                dim=0,
            )
        if pre_avg:
            ...
            with torch.enable_grad():
                target_embeddings = (
                    (target_embeddings * target_freq_inv_norm.unsqueeze(dim=-1)).sum(
                        dim=1
                    )
                    / (
                        0.000000000001
                        + target_freq_inv_norm.sum(dim=1).unsqueeze(dim=-1)
                    )
                ).unsqueeze(dim=1)
                target_emb_neg = (
                    (target_emb_neg * target_freq_inv_norm_neg.unsqueeze(dim=-1)).sum(
                        dim=1
                    )
                    / (
                        0.000000000001
                        + target_freq_inv_norm_neg.sum(dim=1).unsqueeze(dim=-1)
                    )
                ).unsqueeze(dim=1)
        ...
        if coeff_opt == "prod":
            ...
            # iter_coeff = 100
            with torch.enable_grad():
                coeff_mat = estimate_prod_coeff_mat_batch_opt(
                ...
                coeff_mat_neg = estimate_prod_coeff_mat_batch_opt(
                    target_emb_neg.detach(),
                    basis_pred.detach(),
                    L1_losss_B,
                    device,
                    coeff_opt_algo,
            ...
            if coeff_opt_algo == "sgd_bmm":
                coeff_mat = estimate_coeff_mat_batch(
                    target_embeddings.detach(), basis_pred.detach(), L1_losss_B, device
                )
                coeff_mat_neg = estimate_coeff_mat_batch(
                    target_emb_neg.detach(), basis_pred.detach(), L1_losss_B, device
                )
            else:
                lr_coeff = 0.05
                iter_coeff = 60
                with torch.enable_grad():
                    coeff_mat = estimate_coeff_mat_batch_opt(
                        target_embeddings.detach(),
                        basis_pred.detach(),
                        L1_losss_B,
                        device,
                    ...
                    coeff_mat_neg = estimate_coeff_mat_batch_opt(
                        target_emb_neg.detach(),
                        basis_pred.detach(),
                        L1_losss_B,
                        device,
            ...
            coeff_mat = estimate_coeff_mat_batch_max(
                target_embeddings.detach(),
                basis_pred.detach(),
                device,
                loss_type,
            ...
            coeff_mat_neg = estimate_coeff_mat_batch_max(
                target_emb_neg.detach(),
                basis_pred.detach(),
                device,
                loss_type,
            ...
            coeff_mat = estimate_coeff_mat_batch_softmax(
                target_embeddings.detach(),
                basis_pred.detach(),
                device,
                loss_type,
            ...
            coeff_mat_neg = estimate_coeff_mat_batch_softmax(
                target_emb_neg.detach(),
                basis_pred.detach(),
                device,
                loss_type,
    ...
    if loss_type == "sg":
        loss_set = -torch.mean(
            target_freq_inv_norm
            * torch.log(
                torch.sigmoid(torch.sum(pred_embeddings * target_embeddings, dim=2))
            )
        )
        loss_set_neg = -torch.mean(
            target_freq_inv_norm_neg
            * torch.log(
                torch.sigmoid(-torch.sum(pred_embeddings_neg * target_emb_neg, dim=2))
            )
        )
    elif loss_type == "sim":
        loss_set = torch.mean(
            target_freq_inv_norm
            * torch.pow(1 - torch.sum(pred_embeddings * target_embeddings, dim=2), 2)
        )
        loss_set_neg = torch.mean(
            target_freq_inv_norm_neg
            * torch.pow(0 - torch.sum(pred_embeddings_neg * target_emb_neg, dim=2), 2)
        )
    elif loss_type == "dist":
        loss_set = torch.mean(
            target_freq_inv_norm
            * torch.pow(torch.norm(pred_embeddings - target_embeddings, dim=2), 2)
        )
        loss_set_neg = -torch.mean(
            target_freq_inv_norm_neg
            * torch.pow(torch.norm(pred_embeddings_neg - target_emb_neg, dim=2), 2)
        )
    elif loss_type == "dist_expand":
        loss_set = torch.mean(
            target_freq_inv_norm
            * (
                torch.sum(pred_embeddings * pred_embeddings, dim=2)
                - 2 * torch.sum(pred_embeddings * target_embeddings, dim=2)
            )
        )
        loss_set_neg = -torch.mean(
            target_freq_inv_norm_neg
            * (
                torch.sum(pred_embeddings_neg * pred_embeddings_neg, dim=2)
                - 2 * torch.sum(pred_embeddings_neg * target_emb_neg, dim=2)
            )
        )
    ...
    if compute_div_reg:
        ...
        if target_norm:
            ...
            target_emb_neg_norm = target_emb_neg / (
                0.000000000001 + target_emb_neg.norm(dim=2, keepdim=True)
            )
        mask = (target_set > 0).unsqueeze(dim=-1)
        pred_mean = (
            target_embeddings_norm.sum(dim=1, keepdim=True)
            / (
                0.000000000001
                + torch.sum(mask, dim=1, keepdim=True).to(
                    device=target_embeddings_norm.device
                )
            ).detach()
        )
        loss_set_div_target = -torch.sum(
            ((target_embeddings_norm - pred_mean) * mask).norm(dim=2)
        ) / (0.000000000001 + torch.sum(mask))
        assert rand_neg_method == "shuffle"
        mask = (target_index_shuffle > 0).unsqueeze(dim=-1)
        pred_mean = (
            target_emb_neg_norm.sum(dim=1, keepdim=True)
            / (
                0.000000000001
                + torch.sum(mask, dim=1, keepdim=True).to(
                    device=target_emb_neg_norm.device
                )
            ).detach()
        )
        loss_set_div_target += -torch.sum(
            ((target_emb_neg_norm - pred_mean) * mask).norm(dim=2)
        ) / (0.000000000001 + torch.sum(mask))
