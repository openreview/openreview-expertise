...
from .nsd_loss import (
    estimate_prod_coeff_mat_batch_opt,
    estimate_coeff_mat_batch_opt,
    estimate_coeff_mat_batch_max,
)
...
def add_model_arguments(parser):
    ...
    parser.add_argument(
        "--en_model", type=str, default="TRANS", help="type of encoder model (LSTM)"
    )
    # parser.add_argument('--emsize', type=int, default=300,
    #                    help='size of word embeddings')
    parser.add_argument(
        "--source_emsize", type=int, default=200, help="size of word embeddings"
    )
    parser.add_argument(
        "--target_emsize", type=int, default=0, help="size of word embeddings"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--n_basis", type=int, default=3, help="number of basis we want to predict"
    )
    parser.add_argument(
        "--positional_option",
        type=str,
    ...
    parser.add_argument(
        "--dropout_prob_lstm", type=float, default=0, help="dropout_prob in LSTM"
    )

    # scoring
    parser.add_argument(
        "--coeff_opt", type=str, default="max", help="Could be prod or lc or max"
    )
    parser.add_argument(
        "--loss_type", type=str, default="sim", help="Could be sim or dist"
    )


def predict_batch_simple(
    feature, feature_type, parallel_encoder, parallel_decoder, normalize_emb=True
):
    ...
    if normalize_emb:
        ...
        basis_norm_pred_auto = basis_pred_auto / (
            0.000000000001 + basis_pred_auto.norm(dim=2, keepdim=True)
        )
    else:
        basis_norm_pred = basis_pred
        basis_norm_pred_tag = basis_pred_tag
...
def predict_batch(
    feature, feature_type, parallel_encoder, parallel_decoder, word_norm_emb, top_k
):
    (
        basis_norm_pred,
        basis_norm_pred_tag,
        ...
        output_emb_last,
        output_emb,
    ) = predict_batch_simple(feature, feature_type, parallel_encoder, parallel_decoder)
    ...
    for i_sent in range(len(feature_text)):
        ...
        if word_imp_sim is not None:
            ...
            outf.write(
                " ".join(
                    [
                        "\\colorbox{{c{0:02}}}{{{1}}}".format(
                            int(100 - y * 100 / 4.0), x
                        )
                        for x, y in zip(feature_text[i_sent], word_imp_sim[i_sent])
                    ]
                )
                + "\n"
            )
            ...
            for k in range(top_k):
                ...
                if len(word_d2_vis) > 0 and word_nn in word_d2_vis:
                    outf.write(
                        ...
                        + ","
                        + word_d2_vis[word_nn]
                        + " {:5.3f}".format(top_value[i_sent, k, org_ind].item())
                        + " "
                    )
                else:
                    outf.write(
...
def compute_all_dist(
    ...
    def compute_dist(user_norm_emb, basis_norm_pred, coeff_opt, loss_type, device):
        ...
        with torch.enable_grad():
            if coeff_opt == "prod":
                coeff_mat_user = estimate_prod_coeff_mat_batch_opt(
                    target_embeddings.detach(),
                    basis_norm_pred.detach(),
                    L1_losss_B,
                    device,
                    coeff_opt_algo,
                    lr_coeff,
                ...
                coeff_mat_user = estimate_coeff_mat_batch_opt(
                    target_embeddings.detach(),
                    basis_norm_pred.detach(),
                    L1_losss_B,
                    device,
                ...
                coeff_mat_user = estimate_coeff_mat_batch_max(
                    target_embeddings.detach(),
                    basis_norm_pred.detach(),
                    device,
                    loss_type,
        ...
        if loss_type == "sim" or loss_type == "sim_norm":
            dist_all_user_tensor = -torch.sum(
                pred_embeddings_user * target_embeddings, dim=2
            )
        elif loss_type == "dist":
            dist_all_user_tensor = torch.pow(
                torch.norm(pred_embeddings_user - target_embeddings, dim=2), 2
            )
    ...
    if test_user:
        ...
        all_dist_user = compute_dist(
            user_norm_emb, input_basis, coeff_opt, loss_type, device
        )
    else:
        all_dist_user = []
    ...
    if test_tag:
        ...
        all_dist_tag = compute_dist(
            tag_norm_emb, input_basis, coeff_opt, loss_type, device
        )
    else:
        all_dist_tag = []
...
def update_user_score_dict(
    user_batch_list, bid_score_batch_list, user_d2_paper_id_score, paper_id
):
...
def gt_rank_from_list(all_dist_user_j, user_batch_list_j, num_special_token):
    user_rank = ss.rankdata(
        all_dist_user_j
    )  # handle the tie cases by average the ranks
    gt_rank_j = []
...
def pred_per_paper(
    ...
    for j in range(bsz):
        ...
        gt_rank_j = gt_rank_from_list(
            dist_j, user_batch_list[j], num_special_token=num_special_token
        )
        ...
        if len(gt_rank_j) > 0:
            ...
            for m in range(len(recall_at_th)):
                ...
                recall_all_user[m].append(
                    np.sum([int(x <= recall_at_th[m]) for x in gt_rank_j]) / weight
                )
...
def inbalance_by_top_choice(paper_user_dist, reviewer_num_per_paper):
    ...
    paper_num_per_reviewer = math.ceil(
        reviewer_num_per_paper * paper_num / float(user_num)
    )
    ...
    for user_id in user_d2_paper_id_score:
        ...
        if np.var(score_list) > 0:
            ...
            if np.var(paper_sim) > 0:
                spearman, sig = ss.spearmanr(
                    score_list, -paper_dist[list(paper_id_list)]
                )
...
def paper_recall_per_user(
    user_d2_paper_id, paper_user_dist, recall_at_th, dist_matrix=True
):
    # if dist_matrix:
    #    user_paper_dist = list(zip(*paper_user_dist))
    ...
    for user_id in user_d2_paper_id:
        ...
        gt_rank_paper = gt_rank_from_list(
            paper_dist, paper_id_list, num_special_token=-1
        )
        for k in range(len(recall_at_th)):
            weight = min(len(gt_rank_paper), recall_at_th[k])
            recall_list_paper[k].append(
                np.sum([int(x <= recall_at_th[k]) for x in gt_rank_paper]) / weight
            )
    ...
    recall_avg_w = [
        np.average(recall_list_paper[k], weights=weight_list_paper[k])
        for k in range(len(recall_at_th))
    ]
...
def extract_category(feature, feature_type, paper_id_list, paper_id_d2_categories):
    ...
    for i in range(bsz):
        ...
        try:
            up_one_layer = (
                len(feature_category[:-2])
                - 1
                - feature_category[:-2][::-1].index(f_sep_id)
            )
        except Exception:
            up_one_layer = 0
        # if up_one_layer == -1:
...
def all_dist_from_recommend(
    ...
    with torch.no_grad():
        ...
        paper_tag_dist = []
        if test_user:
            paper_user_dist = np.empty(
                (len(all_user_tag), len(user_idx2word_freq)), dtype=np.float16
            )
        if test_tag:
            paper_tag_dist = np.empty(
                (len(all_user_tag), len(tag_idx2word_freq)), dtype=np.float16
            )
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            all_dist_user, all_dist_tag = compute_all_dist(
                feature,
                feature_type,
                parallel_encoder,
                parallel_decoder,
...
def recommend_test_from_all_dist(
    dataloader_info,
    paper_user_dist,
    paper_tag_dist,
    idx2word_freq,
    ...
    with torch.no_grad():
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            bid_score_batch_list = [
                all_user_tag[paper_id][2] for paper_id in paper_id_list
            ]

            if div_eval == "amazon":
                extract_category(
                    feature, feature_type, paper_id_list, paper_id_d2_categories
                )
            ...
            if test_user:
                ...
                top_user_text = convert_feature_to_text(
                    top_prediction_user, user_idx2word_freq
                )
                if most_popular_baseline:
                    (
                        p_gt_rank_user,
                        p_top_prediction_user,
                        p_top_values_user,
                        p_MAP_list_user,
                        p_AUC_list_user,
                        p_NDCG_list_user,
            ...
            if test_tag:
                ...
                top_tag_text = convert_feature_to_text(
                    top_prediction_tag, tag_idx2word_freq
                )
                if most_popular_baseline:
                    (
                        p_gt_rank_tag,
                        p_top_prediction_tag,
                        p_top_values_tag,
                        p_MAP_list_tag,
                        p_AUC_list_tag,
                        p_NDCG_list_tag,
            ...
            for j in range(bsz):
                ...
                if test_user:
                    ...
                    update_user_score_dict(
                        user_batch_list[j],
                        bid_score_batch_list[j],
                        user_d2_paper_id_score,
                        paper_id,
                    )
        ...
        if test_user:
            print(
                "\nUser recall per paper at {} is {}, weighted recall is {}, MAP is {}, F1 is {}, AUC is {}, NDCG is {}".format(
                    recall_at_th_str,
                    [np.mean(recall_all_user[m]) for m in range(len(recall_all_user))],
                    [
                        np.average(recall_all_user[m], weights=weight_all_user[m])
                        for m in range(len(recall_all_user))
                    ],
                    np.mean(MAP_all_user),
                    np.mean(F1_all_user),
                    np.mean(AUC_all_user),
                    np.mean(NDCG_all_user),
                )
            )
            user_id_l2_pred_paper_dist = np.sum(
                paper_user_dist[:, num_special_token:], axis=0
            )
            user_id_l2_pred_paper_dist_mean = np.mean(user_id_l2_pred_paper_dist)
            print(
                "Average absolute deviation on predicted distance {} per paper".format(
                    np.mean(
                        np.absolute(
                            user_id_l2_pred_paper_dist - user_id_l2_pred_paper_dist_mean
                        )
                    )
                    / np.absolute(user_id_l2_pred_paper_dist_mean)
                )
            )
            (
                recall_avg_user,
                ...
                NDCG_user,
                F1_user,
            ) = paper_recall_per_user(user_d2_paper_id, paper_user_dist, recall_at_th)
            print(
                "Paper recall per user at {} is {}, weighted recall is {}, MAP is {}, F1 is {}, AUC is {}, NDCG is {}, popularity correlation is {}".format(
                    ...
                    AUC_user,
                    NDCG_user,
                    ss.pearsonr(paper_id_l2_neg_user_freq, paper_id_l2_pred_user_dist)[
                        0
                    ],
                )
            )
            if len(user_d2_paper_id_score) > 0:
                spearman_score_avg = bid_score_rank_eval(
                    user_d2_paper_id_score, paper_user_dist
                )
                print(
                    "Bid score Spearman correlation coefficient {}".format(
                        spearman_score_avg
                    )
                )
            paper_id_l2_pred_user_dist_mean = np.mean(paper_id_l2_pred_user_dist)
            print(
                "Average absolute deviation on predicted distance {} per user".format(
                    np.mean(
                        np.absolute(
                            paper_id_l2_pred_user_dist - paper_id_l2_pred_user_dist_mean
                        )
                    )
                    / np.absolute(paper_id_l2_pred_user_dist_mean)
                )
            )
            if len(figure_name) > 0:
                ...
                plt.bar(
                    range(len(paper_id_l2_pred_user_dist)),
                    np.sort(paper_id_l2_pred_user_dist),
                )
                plt.savefig(figure_name + "_user_dist_sum_per_paper.png")
                plt.figure()
                plt.bar(
                    range(user_id_l2_pred_paper_dist.shape[0]),
                    np.sort(user_id_l2_pred_paper_dist),
                )
                plt.savefig(figure_name + "_paper_dist_sum_per_user.png")

            if div_eval == "openreview":
                reviewer_num_per_paper = 4
                var_avg, paper_num_per_reviewer = inbalance_by_top_choice(
                    paper_user_dist, reviewer_num_per_paper
                )
                print(
            ...
            if len(paper_id_d2_categories) > 0:
                div_avg, div_course_avg, ent_avg, ent_course_avg = div_by_categories(
                    paper_user_dist, paper_id_d2_categories, div_th
                )
                print(
            ...
            if len(paper_id_d2_tags) > 0:
                ...
                print(
                    "Diversification metric of user by tag difference at {} is {}".format(
                        div_th_str, div_avg
                    )
                )
            if most_popular_baseline:
                print(
                    "Popularity GT baseline. User recall per paper at {} is {}, weighted recall is {}, MAP is {}, F1 is {}, AUC is {}, NDCG is {}".format(
                        recall_at_th_str,
                        [
                            np.mean(p_recall_all_user[m])
                            for m in range(len(p_recall_all_user))
                        ],
                        [
                            np.average(
                                p_recall_all_user[m], weights=p_weight_all_user[m]
                            )
                            for m in range(len(p_recall_all_user))
                        ],
                        np.mean(p_MAP_all_user),
                        np.mean(p_F1_all_user),
                        np.mean(p_AUC_all_user),
                        np.mean(p_NDCG_all_user),
                    )
                )
                (
                    p_recall_avg_user,
                    p_weight_recall_avg_user,
                    p_MAP_user,
                    p_AUC_user,
                    p_NDCG_user,
                    p_F1_user,
                ) = paper_recall_per_user(
                    user_d2_paper_id,
                    paper_id_l2_neg_user_freq,
                    recall_at_th,
                    dist_matrix=False,
                )
                print(
        ...
        if test_tag:
            print(
                "\nTag recall per paper at {} is {}, weighted recall is {}, MAP is {}, F1 is {}, AUC is {}, NDCG is {}".format(
                    recall_at_th_str,
                    [np.mean(recall_all_tag[m]) for m in range(len(recall_all_tag))],
                    [
                        np.average(recall_all_tag[m], weights=weight_all_tag[m])
                        for m in range(len(recall_all_tag))
                    ],
                    np.mean(MAP_all_tag),
                    np.mean(F1_all_tag),
                    np.mean(AUC_all_tag),
                    np.mean(NDCG_all_tag),
                )
            )
            (
                recall_avg_tag,
                ...
                NDCG_tag,
                F1_tag,
            ) = paper_recall_per_user(tag_d2_paper_id, paper_tag_dist, recall_at_th)
            print(
            ...
            if len(paper_id_d2_categories) > 0:
                div_avg, div_course_avg, ent_avg, ent_course_avg = div_by_categories(
                    paper_tag_dist, paper_id_d2_categories, div_th
                )
                print(
            ...
            if most_popular_baseline:
                print(
                    "Popularity baseline. Tag recall per paper at {} is {}, weighted recall is {}, MAP is {}, F1 is {}, AUC is {}, NDCG is {}".format(
                        ...
                        [
                            np.mean(p_recall_all_user[m])
                            for m in range(len(p_recall_all_tag))
                        ],
                        [
                            np.average(p_recall_all_tag[m], weights=p_weight_all_tag[m])
                            for m in range(len(p_recall_all_tag))
                        ],
                        np.mean(p_MAP_all_tag),
                        np.mean(p_F1_all_tag),
                        np.mean(p_AUC_all_tag),
                        np.mean(p_NDCG_all_tag),
                    )
                )
                (
                    p_recall_avg_tag,
...
def recommend_test(
    ...
    most_popular_baseline=True,
    div_eval="openreview",
    switch_user_tag_roles=False,
    store_dist="",
    figure_name="",
):
    paper_user_dist, paper_tag_dist = all_dist_from_recommend(
        dataloader_info,
        parallel_encoder,
        parallel_decoder,
        user_norm_emb,
...
def build_loader_from_pairs(testing_list, sent_d2_topics, bsz, device):
    def store_topics(
        ...
        topic_v, topic_w, sent_emb, avg_word_emb, w_imp_arr, proc_sent = sent_d2_topics[
            sent
        ]
    ...
    if device == "cude":
        use_cuda = True
    testing_pair_loader = torch.utils.data.DataLoader(
        dataset, batch_size=bsz, shuffle=False, pin_memory=use_cuda, drop_last=False
    )
