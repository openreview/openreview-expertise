...
class Dictionary(object):
    ...
    def store_dict(self,f_out):
        ...
        for i in range(vocab_size):
            i_l2_w_freq = [self.ind_l2_w_freq[i][0], str(self.ind_l2_w_freq[i][1]), str(self.ind_l2_w_freq[i][2])] \
...
def load_corpus(data_path, train_bsz, eval_bsz, device, tensor_folder, training_file = "train.pt", split_num = 1, copy_training = False, skip_training = False, want_to_shuffle_val = True, load_val = True,load_test = False, deduplication = False, subsample_ratio = 1, remove_testing_duplication=True):
...
def loading_all_models(args, idx2word_freq, user_idx2word_freq, tag_idx2word_freq, device, max_sent_len, normalize_emb= True, load_auto = False):
    ...
    word_norm_emb = []
    encoder = SEQ2EMB(args.en_model.split('+'), ntokens, source_emb_size, args.nhid, args.nlayers, args.dropout, args.dropouti, args.dropoute, max_sent_len,  word_norm_emb, [], trans_layers = args.encode_trans_layers, trans_nhid = args.trans_nhid, num_type_feature = args.num_type_feature)
