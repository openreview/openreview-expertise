...
class MatrixReconstruction(nn.Module):
    def __init__(self, batch_size, ntopic, nbow, device):
        ...
        self.coeff = nn.Parameter(
            torch.randn(batch_size, ntopic, nbow, device=device, requires_grad=True)
        )
        self.device = device
    ...
    def compute_coeff_pos_norm(self):
        ...
        norm_value = torch.max(
            torch.ones(1, device=self.device), self.coeff.data.sum(dim=2, keepdim=True)
        )
...
class RNN_decoder(nn.Module):
    def __init__(self, model_type, emb_dim, ninp, nhid, nlayers, dropout_prob):
        ...
        if model_type in ["LSTM", "GRU"]:
            ...
            self.rnn = getattr(nn, model_type)(
                emb_dim, nhid, nlayers, dropout=dropout_prob
            )
            ...
            self.rnn = nn.RNN(
                emb_dim, nhid, nlayers, nonlinearity=nonlinearity, dropout=dropout_prob
            )

        if model_type == "LSTM":
            self.init_hid_linear_1 = nn.ModuleList(
                [nn.Linear(ninp, nhid) for i in range(nlayers)]
            )
            self.init_hid_linear_2 = nn.ModuleList(
                [nn.Linear(ninp, nhid) for i in range(nlayers)]
            )
            for i in range(nlayers):
                self.init_hid_linear_1[i].weight.data.uniform_(-0.1, 0.1)
                self.init_hid_linear_1[i].bias.data.uniform_(-0.5, 0.5)
                self.init_hid_linear_2[i].weight.data.uniform_(-0.1, 0.1)
                self.init_hid_linear_2[i].bias.data.uniform_(-0.5, 0.5)
    ...
    def forward(self, input_init, emb):
        hidden_1 = torch.cat(
            [
                self.init_hid_linear_1[i](input_init).unsqueeze(dim=0)
        ...
        hidden_2 = torch.cat(
            [
                self.init_hid_linear_2[i](input_init).unsqueeze(dim=0)
                for i in range(self.nlayers)
            ],
            dim=0,
        )
...
class ext_emb_to_seq(nn.Module):
    def __init__(
        ...
        for model_type in model_type_list:
            if model_type == "LSTM":
                model = RNN_decoder(
                    model_type, input_dim, ninp, nhid, nlayers, dropout_prob_lstm
                )
                input_dim = nhid
                # output_dim = nhid
                ...
                model = Transformer(
                    ...
                    max_position_embeddings=n_basis,
                    num_hidden_layers=trans_layers,
                    add_position_emb=add_position_emb,
                    decoder=using_memory,
                    dropout_prob=dropout_prob_trans,
...
class EMB2SEQ(nn.Module):
    ...
    def __init__(
        ...
        trans_layers=2,
        using_memory=False,
        dropout_prob_trans=0.1,
        dropout_prob_lstm=0,
        de_output_layers="no_dynamic",
    ):
        ...
        if positional_option == "linear":
            ...
            self.init_linear_arr = nn.ModuleList(
                [nn.Linear(ninp, linear_mapping_dim) for i in range(n_basis)]
            )
            for i in range(n_basis):
                ...
                self.init_linear_arr[i].bias.data.uniform_(-0.5, 0.5)
                self.init_linear_arr[i].weight.data.uniform_(-0.1, 0.1)
        ...
        self.dep_learner = ext_emb_to_seq(
            model_type_list,
            input_size,
            ninp,
            nhid,
        ...
        if de_output_layers == "single_dynamic":
            self.out_linear_arr = nn.ModuleList(
                [
                    nn.Linear(self.dep_learner.output_dim, target_emb_sz)
                    for i in range(n_basis)
                ]
            )
        # self.final = nn.Linear(target_emb_sz, target_emb_sz)
        if de_output_layers == "double_dynamic":
            self.final_linear_arr = nn.ModuleList(
                [nn.Linear(target_emb_sz, target_emb_sz) for i in range(n_basis)]
            )
        ...
        if coeff_model == "LSTM":
            ...
            self.coeff_rnn = nn.LSTM(
                input_size + target_emb_sz,
                nhid,
                num_layers=coeff_nlayers,
                bidirectional=True,
            )
            output_dim = nhid * 2
        elif (
            coeff_model == "TRANS"
            or coeff_model == "TRANS_old"
            or coeff_model == "TRANS_pool"
            or coeff_model == "TRANS_two_heads"
        ):
            coeff_nlayers = 2
            self.coeff_trans = Transformer(
                model_type="TRANS",
                hidden_size=input_size + target_emb_sz,
                max_position_embeddings=n_basis,
                num_hidden_layers=coeff_nlayers,
                add_position_emb=False,
                decoder=False,
                dropout_prob=0.3,
            )
        ...
        if coeff_model != "None":
            ...
            if coeff_model == "TRANS_two_heads":
                ...
                self.coeff_out_linear_2_tag = nn.Linear(
                    half_output_dim, half_output_dim
                )
    ...
    def forward(self, input_init, memory=None, predict_coeff_sum=False):
        ...
        def prepare_posi_emb(input, poistion_emb, drop):
            ...
            input_pos = (
                torch.arange(n_basis, dtype=torch.long, device=input.device)
                .expand(batch_size, n_basis)
                .permute(1, 0)
            )
        ...
        if self.positional_option == "linear":
            emb_raw = torch.cat(
                [
                    self.init_linear_arr[i](input_init).unsqueeze(dim=0)
            ...
            if self.dep_learner.decoder_array[0].model_type == "LSTM":
                poistion_emb_input = prepare_posi_emb(
                    input, self.poistion_emb, self.drop
                )
        ...
        if hasattr(self, "out_linear"):
            ...
            output = torch.cat(
                [
                    self.out_linear_arr[i](output[i, :, :]).unsqueeze(dim=0)
                    for i in range(self.n_basis)
                ],
                dim=0,
            )
        ...
        if hasattr(self, "final_linear_arr"):
            output = torch.cat(
                [
                    self.final_linear_arr[i](output[i, :, :]).unsqueeze(dim=0)
                    for i in range(self.n_basis)
                ],
                dim=0,
            )
        ...
        if self.coeff_model == "TRANS" or self.coeff_model == "TRANS_two_heads":
            ...
            coeff_pred_1 = F.relu(self.coeff_out_linear_1(coeff_output))
            coeff_pred_2 = F.relu(self.coeff_out_linear_2(coeff_pred_1))
            coeff_pred = self.coeff_out_linear_3(
                coeff_pred_2.view(coeff_pred_2.size(0), -1)
            )
...
class RNN_encoder(nn.Module):
    def __init__(self, model_type, ninp, nhid, nlayers, dropout):
        ...
        if model_type in ["LSTM", "GRU"]:
            self.rnn = getattr(nn, model_type)(
                ninp, nhid, nlayers, dropout=0, bidirectional=True
            )
        else:
            try:
                nonlinearity = {"RNN_TANH": "tanh", "RNN_RELU": "relu"}[model_type]
...
class RNN_pooler(nn.Module):
    ...
    def forward(self, output, bsz):
        ...
        output_last = torch.cat(
            (output_unpacked[-1, :, 0, :], output_unpacked[0, :, 1, :]), dim=1
        )
    ...
    def __init__(
        ...
        for i, model_type in enumerate(model_type_list):
            if model_type == "LSTM" or model_type == "GRU":
                ...
                model = Transformer(
                    model_type=model_type,
                    hidden_size=trans_nhid,
                    max_position_embeddings=max_sent_len,
                    num_hidden_layers=trans_layers,
                    add_position_emb=add_position_emb,
                    num_type_feature=num_type_feature,
...
class SEQ2EMB(nn.Module):
    ...
    def __init__(
        ...
        if len(external_emb) > 1 and ninp == 0:
            ...
            self.encoder = nn.Embedding.from_pretrained(
                external_emb.clone() * scale_factor, freeze=False
            )
            ...
            if len(init_idx) > 0:
                ...
                extra_init_emb = torch.randn(
                    len(init_idx), ninp, requires_grad=False, device=device
                )
        ...
        # self.init_weights()
        self.seq_summarizer = seq_to_emb(
            model_type_list,
            ninp,
            nhid,
            nlayers,
    ...
    def forward(self, input, input_type):
        ...
        emb = embedded_dropout(
            self.encoder, input.t(), dropout=self.dropoute if self.use_dropout else 0
        )
