...
class _PredictManagerCustom(_PredictManager):
    ...
    def __init__(
        ...
        super(_PredictManagerCustom, self).__init__(
            predictor,
            input_file,
            output_file,
            batch_size,
        ...
        self.total_size = int(
            sum([1 for _ in open(self._input_file)]) / self._batch_size
        )
    ...
    def run(self) -> None:
        ...
        if has_reader:
            for batch in tqdm(
                lazy_groups_of(self._get_instance_data(), self._batch_size),
...
def predictor_from_archive(
    archive: Archive, predictor_name: str = None, paper_features_path: str = None
) -> "Predictor":
    """
    Source: https://github.com/allenai/specter/blob/master/scripts/embed.py
...
class SpecterPredictor:
    def __init__(
        ...
        ), "(Only) One of max_score or average_score must be True"
    ...
    def set_archives_dataset(self, archives_dataset):
        ...
        with open(
            os.path.join(self.work_dir, "specter_reviewer_paper_data.json"), "w"
        ) as f_out:
            json.dump(output_dict, f_out, indent=1)
        with open(
            os.path.join(self.work_dir, "specter_reviewer_paper_ids.txt"), "w"
        ) as f_out:
            f_out.write("\n".join(paper_ids_list) + "\n")
    ...
    def set_submissions_dataset(self, submissions_dataset):
        ...
        for note_id, submission in submissions_dataset.items():
            ...
            output_dict[submission["id"]] = {
                "title": self.sub_note_id_to_title[submission["id"]],
                "abstract": self.sub_note_id_to_abstract[submission["id"]],
                "paper_id": submission["id"],
                "authors": [],
            }
        with open(
            os.path.join(self.work_dir, "specter_submission_paper_data.json"), "w"
        ) as f_out:
            json.dump(output_dict, f_out, indent=1)
        with open(
            os.path.join(self.work_dir, "specter_submission_paper_ids.txt"), "w"
        ) as f_out:
            f_out.write("\n".join(paper_ids_list) + "\n")
    ...
    def embed_submissions(self, submissions_path=None):
        ...
        metadata_file = os.path.join(
            self.work_dir, "specter_submission_paper_data.json"
        )
        ...
        overrides = json.dumps(
            {
                ...
                "dataset_reader": {
                    ...
                    "cache_path": os.path.join(
                        self.specter_dir, "data/dataset-instance-cache/"
                    ),
        ...
        manager = _PredictManagerCustom(
            predictor, ids_file, submissions_path, self.batch_size, False, False
        )
        manager.run()
    ...
    def embed_publications(self, publications_path=None):
        ...
        manager = _PredictManagerCustom(
            predictor, ids_file, publications_path, self.batch_size, False, False
        )
        manager.run()
    ...
    def all_scores(
        ...
        def load_emb_file(emb_file):
            ...
            emb_tensor = emb_tensor / (
                emb_tensor.norm(dim=1, keepdim=True) + 0.000000000001
            )
        ...
        p2p_aff = torch.empty(
            (paper_num_test, paper_num_train), device=torch.device("cpu")
        )
        for i in range(paper_num_test):
            p2p_aff[i, :] = torch.sum(
                paper_emb_test[i, :].unsqueeze(dim=0) * paper_emb_train, dim=1
            )
        ...
        for reviewer_id, train_note_id_list in self.pub_author_ids_to_note_id.items():
            ...
            for j in range(paper_num_test):
                csv_line = "{note_id},{reviewer},{score}".format(
                    note_id=test_id_list[j],
                    reviewer=reviewer_id,
                    score=all_paper_aff[j].item(),
                )
                csv_scores.append(csv_line)
                self.preliminary_scores.append(
                    (test_id_list[j], reviewer_id, all_paper_aff[j].item())
                )
