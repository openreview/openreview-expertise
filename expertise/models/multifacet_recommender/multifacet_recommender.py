...
from .mfr_src.utils import (
    seed_all_randomness,
    Dictionary,
    save_checkpoint,
    load_emb_file_to_dict,
    load_emb_file_to_tensor,
    load_corpus,
    output_parallel_models,
    load_emb_from_path,
)
...
def squeeze_into_tensors(
    ...
    def save_to_tensor(w_ind_corpus_dup_j, out_tensor, idx, store_right=False):
        ...
        if store_right:
            out_tensor[idx, -sent_len:] = torch.tensor(
                w_ind_corpus_dup_j, dtype=store_type
            )
        else:
            out_tensor[idx, :sent_len] = torch.tensor(
                w_ind_corpus_dup_j, dtype=store_type
    ...
    def duplicate_for_long_targets(
        save_idx,
        w_ind_corpus,
        type_corpus,
        user_corpus,
...
def counter_to_tensor(idx2word_freq, device, uniform=True, smooth_alpha=0.0):
    ...
    if len(emb_file) > 0:
        if emb_file[-3:] == ".pt":
            ...
            target_emb = torch.randn(
                num_w, target_emb_sz, device=device, requires_grad=False
            )
            OOV_freq = 0
            total_freq = 0
            OOV_type = 0
            ...
            print(
                "OOV token percentage: {}%".format(OOV_freq / float(total_freq) * 100)
            )
    else:
        target_emb = torch.randn(
            num_w, target_emb_sz, device=device, requires_grad=False
...
class MultiFacetRecommender(object):
    def __init__(
        ...
        copyfile(
            feature_vocab_file, os.path.join(work_dir, "feature", "dictionary_index")
        )
    ...
    def set_archives_dataset(self, archives_dataset):
        ...
        for profile_id, publications in archives_dataset.items():
            for publication in publications:
                ...
                self.pub_note_id_to_title[publication["id"]] = publication[
                    "content"
                ].get("title")
    ...
    def set_submissions_dataset(self, submissions_dataset):
        ...
        for note_id, submission in submissions_dataset.items():
            ...
            self.sub_note_id_to_title[submission["id"]] = submission["content"].get(
                "title"
            )
            if self.sub_note_id_to_title[submission["id"]] is None:
                self.sub_note_id_to_title[submission["id"]] = ""
            self.sub_note_id_to_abstract[submission["id"]] = submission["content"].get(
                "abstract"
            )
    ...
    @staticmethod
    def _map_tokens_to_indices(
        corpus, input_vocab, lowercase, ignore_unk, eos, save_file_name=""
    ):
        w_ind_corpus = []
        ...
        for w_ind_list in w_ind_corpus:
            compact_w_ind_corpus.append(
                [compact_mapping[x] for x in w_ind_list if compact_mapping[x] > 0]
            )
            ...
            with open(save_file_name, "w") as f_out:
                for w_ind_list in w_ind_corpus:
                    f_out.write(
                        " ".join(
                            [
                                str(compact_mapping[x])
                                for x in w_ind_list
                                if compact_mapping[x] > 0
                            ]
                        )
                        + "\n"
                    )
    ...
    def _build_dictionary(corpus, save_path, lowercase, min_freq=0, ignore_unk=True):
        ...
        for w_list_org in corpus:
            for w in w_list_org:
                ...
                total_num_w += 1
        compact_mapping, total_freq_filtering = dict_c.densify_index(
            min_freq, ignore_unk
        )
    ...
    def _shorten_input_features(indices, max_sent_len, push_to_right=True):
        ...
        print(
            "Finish loading {} sentences. While truncating {} long sentences".format(
                len(out_indices), num_too_long_sent
            )
        )
        return out_indices
    ...
    def _train_one_epoch(
        ...
        log_interval=200,
        L1_losss_B=0.2,
        norm_basis_when_freezing=False,
        user_uniform=None,
        user_freq=None,
        tag_uniform=None,
        tag_freq=None,
        basis_pred_train_cache=None,
        basis_pred_tag_train_cache=None,
    ):
        start_time = time.time()
        total_loss = 0.0
        total_loss_set_user = 0.0
        total_loss_set_neg_user = 0.0
        total_loss_set_tag = 0.0
        total_loss_set_neg_tag = 0.0
        total_loss_set_auto = 0.0
        total_loss_set_neg_auto = 0.0
        total_loss_set_reg = 0.0
        total_loss_set_div = 0.0
        total_loss_set_div_target_user = 0.0
        total_loss_set_div_target_tag = 0.0
        ...
        for i_batch, sample_batched in enumerate(dataloader_train):
            ...
            if freeze_encoder_decoder and epoch > 1:
                ...
                basis_pred = (
                    basis_pred_train_cache[sample_idx_np, :, :].detach().clone()
                )
                basis_pred_tag = (
                    basis_pred_tag_train_cache[sample_idx_np, :, :].detach().clone()
                )
            else:
                output_emb_last, output_emb = parallel_encoder(feature, feature_type)

                basis_pred, basis_pred_tag, basis_pred_auto = parallel_decoder(
                    output_emb_last, output_emb, predict_coeff_sum=False
                )
                if freeze_encoder_decoder:
                    # store cache
                    if norm_basis_when_freezing:
                        basis_pred = basis_pred / (
                            0.000000000001 + basis_pred.norm(dim=2, keepdim=True)
                        )
                        basis_pred_tag = basis_pred_tag / (
                            0.000000000001 + basis_pred_tag.norm(dim=2, keepdim=True)
                        )
                    sample_idx_np = sample_idx.numpy()
                    basis_pred_train_cache[
                        sample_idx_np, :, :
                    ] = basis_pred.detach().clone()
                    basis_pred_tag_train_cache[
                        sample_idx_np, :, :
                    ] = basis_pred_tag.detach().clone()
            ...
            if auto_w > 0:
                feature_len = None
                if auto_avg:
                    basis_pred_auto_compressed = basis_pred_auto.mean(dim=1).unsqueeze(
                        dim=1
                    )
                else:
                    basis_pred_auto_compressed = basis_pred_auto
                rand_neg_method = "rotate"
                loss_set_auto, loss_set_neg_auto = nsd_loss.compute_loss_set(
            ...
            total_loss_set_neg_user += (
                loss_set_neg_user.item() * small_batch_size / batch_size
            )
            total_loss_set_tag += loss_set_tag.item() * small_batch_size / batch_size
            total_loss_set_neg_tag += (
                loss_set_neg_tag.item() * small_batch_size / batch_size
            )
            total_loss_set_auto += loss_set_auto.item() * small_batch_size / batch_size
            total_loss_set_neg_auto += (
                loss_set_neg_auto.item() * small_batch_size / batch_size
            )
            ...
            total_loss_set_div_target_tag += (
                loss_set_div_target_tag.item() * small_batch_size / batch_size
            )
            total_loss_set_div_target_user += (
                loss_set_div_target_user.item() * small_batch_size / batch_size
            )
            ...
            if i_batch % log_interval == 0 and i_batch > 0:
                ...
                cur_loss_set_div_target_user = (
                    total_loss_set_div_target_user / log_interval
                )
                cur_loss_set_div_target_tag = (
                    total_loss_set_div_target_tag / log_interval
                )
                elapsed = time.time() - start_time
                logging(
                    "| e {:3d} {:3d} | {:5d}/{:5d} b | lr-enc {:.6f} | ms/batch {:5.2f} | "
                    "l {:5.2f} | l_f_u {:5.5f} + {:2.2f}*{:5.5f} = {:5.5f} | div_u {:5.2f} | l_f_t {:5.4f} + {:2.2f}*{:5.4f} = {:5.4f} | div_t {:5.2f} | l_f_a {:5.4f} + {:2.2f}*{:5.4f} = {:5.4f} | reg {:5.2f} | div {:5.2f} ".format(
                        ...
                        split_i,
                        i_batch,
                        len(dataloader_train.dataset) // batch_size,
                        optimizer_e.param_groups[0]["lr"],
                        elapsed * 1000 / log_interval,
                        cur_loss,
                        cur_loss_set_user,
                        neg_sample_w,
                        cur_loss_set_neg_user,
                        cur_loss_set_user + neg_sample_w * cur_loss_set_neg_user,
                        cur_loss_set_div_target_user,
                        cur_loss_set_tag,
                        neg_sample_w,
                        cur_loss_set_neg_tag,
                        cur_loss_set_tag + neg_sample_w * cur_loss_set_neg_tag,
                        cur_loss_set_div_target_tag,
                        cur_loss_set_auto,
                        neg_sample_w,
                        cur_loss_set_neg_auto,
    ...
    def embed_submissions(self, submissions_path=None):
        ...
        titles_corpus = [
            self.sub_note_id_to_title[note_id]
            for note_id in self.submission_paper_ids_list
        ]
        titles_corpus_tokens = self._tokenize_text(titles_corpus)
        abstracts_corpus = [
            self.sub_note_id_to_abstract[note_id]
            for note_id in self.submission_paper_ids_list
        ]
        abstracts_corpus_tokens = self._tokenize_text(abstracts_corpus)
        meta_corpus, type_indices = self._concat_title_abstract(
            titles_corpus_tokens, abstracts_corpus_tokens
        )
        feature_indices = self._map_tokens_to_indices(
            meta_corpus,
            os.path.join(self.work_dir, "feature", "dictionary_index"),
            lowercase=True,
            ignore_unk=False,
            eos=True,
            save_file_name=os.path.join(self.work_dir, "feature", "corpus_index_test"),
        )
        feature_indices_trunc = self._shorten_input_features(
            feature_indices, max_sent_len=512
        )
        type_indices_trunc = self._shorten_input_features(
            type_indices, max_sent_len=512
        )
        ...
        squeeze_into_tensors(
            all_idx,
            feature_indices_trunc,
            type_indices_trunc,
            user_indices,
            tag_indices,
            [],
            os.path.join(tensor_output_dir, "test.pt"),
            shuffle_order=False,
        )

        print(
            "NOTE: Currently submission embeddings cannot be precomputed. "
    ...
    def embed_publications(self, publications_path=None):
        ...
        titles_corpus = [
            self.pub_note_id_to_title[note_id]
            for note_id in self.archive_paper_ids_list
        ]
        titles_corpus_tokens = self._tokenize_text(titles_corpus)
        abstracts_corpus = [
            self.pub_note_id_to_abstract[note_id]
            for note_id in self.archive_paper_ids_list
        ]
        abstracts_corpus_tokens = self._tokenize_text(abstracts_corpus)
        meta_corpus, type_indices = self._concat_title_abstract(
        ...
        feature_indices = self._map_tokens_to_indices(
            ...
            ignore_unk=False,
            eos=True,
            save_file_name=os.path.join(self.work_dir, "feature", "corpus_index"),
        )
        feature_indices_trunc = self._shorten_input_features(
            feature_indices, max_sent_len=512
        )
        ...
        user_dictionary = os.path.join(self.work_dir, "user", "dictionary_index")
        user_corpus = [
            self.pub_note_id_to_author_ids[note_id]
            for note_id in self.archive_paper_ids_list
        ]
        if not os.path.exists(user_dictionary):
            ...
            self._build_dictionary(
                corpus=user_corpus,
                save_path=os.path.join(self.work_dir, "user"),
                lowercase=False,
            )
        user_indices = self._map_tokens_to_indices(
            user_corpus,
            user_dictionary,
            lowercase=False,
            ignore_unk=True,
            eos=False,
            save_file_name=os.path.join(self.work_dir, "user", "corpus_index"),
        )
        ...
        if not os.path.exists(tag_dictionary):
            ...
            self._build_dictionary(
                corpus=tag_corpus,
                save_path=os.path.join(self.work_dir, "tag"),
                lowercase=False,
            )
        tag_indices = self._map_tokens_to_indices(
            tag_corpus,
            tag_dictionary,
            lowercase=False,
            ignore_unk=True,
            eos=False,
            save_file_name=os.path.join(self.work_dir, "tag", "corpus_index"),
        )
        ...
        squeeze_into_tensors(
            ...
            tag_indices,
            [],
            os.path.join(tensor_output_dir, "train.pt"),
            shuffle_order=False,
        )
        ...
        source_emb = torch.tensor([0.0])
        extra_init_idx = []
        source_emb, source_emb_size, extra_init_idx = load_emb_file_to_tensor(
            "/dev/null", self.device, idx2word_freq
        )
        source_emb = source_emb / (
            0.000000000001 + source_emb.norm(dim=1, keepdim=True)
        )
        source_emb.requires_grad = False

        user_emb, target_emb_sz = load_ext_emb(
            self.user_emb_file,
            self.target_emsize,
            user_idx2word_freq,
            num_special_token,
        ...
        if self.tag_w > 0:
            tag_emb, target_emb_sz_tag = load_ext_emb(
                self.tag_emb_file,
                self.target_emsize,
                tag_idx2word_freq,
                num_special_token,
        ...
        if not self.inv_freq_w:
            user_uniform = counter_to_tensor(
                user_idx2word_freq, self.device, uniform=True
            )
            tag_uniform = counter_to_tensor(
                tag_idx2word_freq, self.device, uniform=True
            )
        else:
            user_uniform = counter_to_tensor(
                user_idx2word_freq, self.device, uniform=False
            )
            tag_uniform = counter_to_tensor(
                tag_idx2word_freq, self.device, uniform=False
            )
        ...
        if self.auto_w > 0:
            ...
            feature_uniform = counter_to_tensor(
                idx2word_freq, self.device, uniform=False, smooth_alpha=1e-4
            )
            feature_freq = counter_to_tensor(idx2word_freq, self.device, uniform=False)
            feature_linear_layer = torch.randn(
                source_emb_size, target_emb_sz, device=self.device, requires_grad=True
            )
        else:
        ...
        decoder = model_code.EMB2SEQ(
            ...
            1,
            self.n_basis,
            positional_option=self.positional_option,
            dropoutp=self.dropoutp,
            trans_layers=self.trans_layers,
            using_memory=self.de_en_connection,
            dropout_prob_trans=self.dropout_prob_trans,
            dropout_prob_lstm=self.dropout_prob_lstm,
            de_output_layers=self.de_output_layers,
        )

        if (
            self.de_en_connection
            and decoder.trans_dim is not None
            and encoder.output_dim != decoder.trans_dim
        ):
            print(
                "dimension mismatch. The encoder output dimension is ",
                encoder.output_dim,
                " and the transformer dimension in decoder is ",
                decoder.trans_dim,
            )
            sys.exit(1)
        ...
        if self.continue_train:
            encoder.load_state_dict(
                torch.load(
                    os.path.join(self.model_checkpoint_dir, "encoder.pt"),
                    map_location=self.device,
                )
            )
            decoder.load_state_dict(
                torch.load(
                    os.path.join(self.model_checkpoint_dir, "decoder.pt"),
                    map_location=self.device,
                )
            )
            if self.loading_target_embedding:
                user_emb_load = torch.load(
                    os.path.join(self.model_checkpoint_dir, "user_emb.pt"),
                    map_location=self.device,
                )
                user_emb = user_emb.new_tensor(user_emb_load)
                if self.tag_w > 0:
                    tag_emb_load = torch.load(
                        os.path.join(self.model_checkpoint_dir, "tag_emb.pt"),
                        map_location=self.device,
                    )
                    tag_emb = tag_emb.new_tensor(tag_emb_load)

        parallel_encoder, parallel_decoder = output_parallel_models(
            self.use_cuda, self.single_gpu, encoder, decoder
        )

        total_params = sum(x.data.nelement() for x in encoder.parameters())
        logging(
            "Encoder total parameters: {}".format(total_params),
            self.work_dir,
            self.log_file_name,
        )
        total_params = sum(x.data.nelement() for x in decoder.parameters())
        logging(
            "Decoder total parameters: {}".format(total_params),
            self.work_dir,
            self.log_file_name,
        )

        optimizer_e = torch.optim.Adam(
            encoder.parameters(), lr=self.lr, weight_decay=self.wdecay
        )
        optimizer_d = torch.optim.Adam(
            decoder.parameters(), lr=self.lr / self.lr2_divide, weight_decay=self.wdecay
        )
        optimizer_t = torch.optim.Adam(
            [user_emb, tag_emb], lr=self.lr_target, weight_decay=self.target_l2
        )
        ...
        basis_pred_train_cache = torch.empty(
            (num_sample_train, self.n_basis, target_emb_sz),
            dtype=torch.float,
            device=self.device,
        )
        basis_pred_tag_train_cache = torch.empty(
            (num_sample_train, self.n_basis, target_emb_sz),
            dtype=torch.float,
            device=self.device,
        )
        ...
        for epoch in range(1, self.epochs + 1):
            for i in range(len(dataloader_train_arr)):
                if epoch == 1 and i < self.start_training_split:
                    ...
                    continue
                current_coeff_opt = self._train_one_epoch(
                    encoder,
                    decoder,
                    parallel_encoder,
                    parallel_decoder,
                    ...
                    self.coeff_opt_algo,
                    current_coeff_opt,
                    self.rand_neg_method,
                    self.target_norm,
                    self.freeze_encoder_decoder,
                    self.update_target_emb,
                    self.user_w,
                    self.tag_w,
                    self.auto_w,
                    self.auto_avg,
        ...
        self.user_emb_file = os.path.join(
            self.work_dir, "user_emb" + self.target_embedding_suffix + "_always.pt"
        )
        self.tag_emb_file = os.path.join(
            self.work_dir, "tag_emb" + self.target_embedding_suffix + "_always.pt"
        )
        self.source_emsize = 200

        print("Reviewer embeddings are saved at {}".format(self.user_emb_file))
        print(
            "NOTE: Archive publication embeddings are not explicitly computed. "
            "Reviewer embeddings are computed and consumed with submission embeddings during score calculation"
        )

    def all_scores(
        self, publications_path=None, submissions_path=None, scores_path=None
    ):
        print("Loading data...")
        seed_all_randomness(self.seed, self.use_cuda)
        (
            ...
            self.batch_size,
            self.device,
            tensor_folder=self.tensor_folder,
            skip_training=True,
        ...
        user_emb, user_emb_size = load_emb_from_path(
            self.user_emb_file, self.device, user_idx2word_freq
        )

        tag_emb, tag_emb_size = load_emb_from_path(
            self.tag_emb_file, self.device, tag_idx2word_freq
        )
        ...
        encoder = model_code.SEQ2EMB(
            ...
            external_emb=[],
            init_idx=[],
            trans_layers=self.encode_trans_layers,
            trans_nhid=self.trans_nhid,
        )
        ...
        if normalize_emb:
            ...
            user_norm_emb = user_emb / (
                0.000000000001 + user_emb.norm(dim=1, keepdim=True)
            )
        else:
            tag_norm_emb = tag_emb
            user_norm_emb = user_emb
        ...
        # Compute distances
        with open(
            os.path.join(self.work_dir, "reviewer_submission_dist_arr.txt"), "w"
        ) as dist_file:
            recommend_test(
                dataloader_test_info,
                parallel_encoder,
                ...
                dist_file,
                self.device,
                self.most_popular_baseline,
                self.div_eval,
                self.switch_user_tag_roles,
                self.store_dist,
                figure_name="",
            )
        ...
        for j in range(num_special_token, len(user_idx2word_freq)):
            ...
            for i, paper_id in enumerate(self.submission_paper_ids_list):
                csv_line = "{note_id},{reviewer},{score}".format(
                    note_id=paper_id, reviewer=user_name, score=sim_arr[i, j]
                )
