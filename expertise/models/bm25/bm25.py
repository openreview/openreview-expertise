...
class Model(object):
        ...
        for profile_id, publications in archives_dataset.items():
            for publication in publications:
                if self.use_abstract and self._is_valid_field(
                    ...
                    tokenized_abstract = (
                        publication["content"]["abstract"].lower().split(" ")
                    )
    ...
    def score(self, submission):
        ...
        if self.use_abstract and self._is_valid_field(
            ...
            submission_scores = torch.tensor(
                self.bm25_abstracts.get_scores(tokenized_abstract), dtype=torch.float32
            )
            ...
            submission_scores = torch.tensor(
                self.bm25_titles.get_scores(tokenized_title), dtype=torch.float32
            )
        else:
            return None
        self.metadata["closest_match"][submission["id"]] = (
            submission,
            self.raw_publications[submission_scores.max(dim=0)[1]],
        )
        ...
        if self.average_score:
            for profile_id, (
                start_index,
                end_index,
            ) in self.profie_id_to_indices.items():
                if start_index == end_index:
                    reviewer_scores[profile_id] = 0.0
                    self.metadata["no_expertise"].add(profile_id)
                else:
                    reviewer_scores[profile_id] = (
                        submission_scores[start_index:end_index].mean().item()
                    )
        elif self.max_score:
            for profile_id, (
                start_index,
                ...
                if start_index == end_index:
                    ...
                    reviewer_scores[profile_id] = (
                        submission_scores[start_index:end_index].max().item()
                    )
    ...
    def all_scores_helper(self, submissions_dataset):
        ...
        for note_id, submission in tqdm(
            submissions_dataset.items(), total=len(submissions_dataset)
        ):
    ...
    def all_scores(self, preliminary_scores_path=None, scores_path=None):
        ...
        for idx, (note_id, submission) in enumerate(self.submissions_dataset.items()):
            ...
            if (
                idx % (len(self.submissions_dataset) // self.workers + 1)
                >= len(self.submissions_dataset) // self.workers
            ):
        ...
        if scores_path:
            with open(scores_path, "w") as f:
                for note_id, profile_id, score in tqdm(
                    self.preliminary_scores, desc="Saving preliminary scores"
                ):
