...
class Evaluator(base_evaluator.Evaluator):
    ...
    def evaluate_using_individual_queries(self, ranklists):
        ...
        for forum, rank_list in ranklists:
            ...
            for m in self.m_values:
                ...
                positive_bids = [
                    bid.signatures[0].encode("utf-8")
                    for bid in self.bids_by_forum[forum]
                    if bid.tag in positive_labels
                ]
                relevant_reviewers = [
                    1 if reviewer_id in positive_bids else 0
                    for reviewer_id in rank_list
                ]
    ...
    def reviewer_has_bid(self, reviewer, paper):
        ...
        has_bid = [
            True if bid.signatures[0] == reviewer.decode("utf-8") else False
            for bid in paper_bids
        ][0]
        return has_bid
    ...
    def get_bid_for_reviewer_paper(self, reviewer, paper):
        ...
        paper_bids = self.bids_by_forum[paper]
        bid_value = [
            1 if bid.tag in positive_labels else 0
            for bid in paper_bids
            if bid.signatures[0] == reviewer.decode("utf-8")
        ]
        if len(bid_value) > 0:
            return bid_value[0]
        else:
    ...
    def evaluate_using_single_rank(self, rank_list):
        ...
        for m in range(1, len(ranked_reviewers) + 1):
            ...
            topM = map(
                lambda reviewer: (
                    reviewer[0],
                    self.get_bid_for_reviewer_paper(reviewer[0], reviewer[2]),
                ),
                topM,
            )
            pos_bids_from_topM = [bid for bid in topM if bid[1] == 1]
            precision = float(len(pos_bids_from_topM)) / float(
                m
            )  # precision => relevant bids retrieved / # of retrieved
    ...
    def graph_precision_values(self, precision_values):
        ...
        df_recall = pd.DataFrame(
            {"@M": range(1, len(precision_values) + 1), "Recall": precision_values}
        )
