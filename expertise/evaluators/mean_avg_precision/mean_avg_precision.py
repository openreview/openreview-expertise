...
I think actually we want to evaluate against the list of reviewers suggested for a paper, rather than the list of papers suggested for a reviewer.
    ...
    def mean_avg_precision(self, ranked_signatures, forum):
        '''
        Among the top M users in the model's rank list,
        compute the MAP.
        Ranked signatures has [('paper_id', ['reviewer_name;affinity', ...]), (...), ...]
        ...
        if not positive_signatures:
            ...
            correct = 0
            total = 0
            for r in ranked_signatures[:10]:
                total += 1
                if r in positive_signatures:
                    correct += 1
    ...
    def evaluate(self, ranklists):
        ...
        for forum, rank_list in ranklists:
            ...
            scores = []
            scores.append(self.mean_avg_precision(ranked_signatures, forum))
            yield forum, scores
