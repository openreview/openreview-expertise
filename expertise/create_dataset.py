...
class OpenReviewExpertise(object):
    ...
    def get_paper_notes(self, author_id, dataset_params):
        ...
        if use_bids_as_expertise:
            ...
            bids = openreview.tools.iterget_edges(
                self.openreview_client, invitation=bid_invitation, tail=author_id
            )
            note_ids = [e.head for e in bids if e.label in ["Very High", "High"]]
            return [
                n
                for n in self.openreview_client.get_notes_by_ids(ids=note_ids)
                if n.invitation == paper_invitation
            ]

        return openreview.tools.iterget_notes(
            self.openreview_client, content={"authorids": author_id}
        )

    def get_publications(self, author_id):

        dataset_params = self.config.get("dataset", {})
        minimum_pub_date = dataset_params.get("minimum_pub_date") or dataset_params.get(
            "or", {}
        ).get("minimum_pub_date", 0)
        top_recent_pubs = dataset_params.get("top_recent_pubs") or dataset_params.get(
            "or", {}
        ).get("top_recent_pubs", False)
        ...
        for publication in publications:
            ...
            if self.config.get("dataset", {}).get("with_abstract", False):
                if (
                    "abstract" not in publication.content
                    or not publication.content.get("abstract")
                ):
                    continue
            if self.config.get("dataset", {}).get("with_title", False):
                if "title" not in publication.content or not publication.content.get(
                    "title"
                ):
                    continue
            # Exclude blind Notes
        ...
        sorted_publications = sorted(
            unsorted_publications, key=lambda pub: pub.get("cdate"), reverse=True
        )
        ...
        if not top_recent_pubs:
            return [
                ...
                if publication["cdate"] >= minimum_pub_date
            ]
    ...
    def get_profile_ids(self, group_ids=None, reviewer_ids=None):
        ...
        profile_search_results = (
            self.openreview_client.search_profiles(
                confirmedEmails=None, ids=tilde_members_list, term=None
            )
            if tilde_members_list
            else []
        )
        tilde_members_list = []
        for profile in profile_search_results:
            ...
            preferredEmail = (
                preferredEmail
                or profile.content.get("emailsConfirmed")
                and len(profile.content.get("emailsConfirmed"))
                and profile.content.get("emailsConfirmed")[0]
            )
            # If the user does not have emails confirmed, use the first email in the emails list
            preferredEmail = (
                preferredEmail
                or profile.content.get("emails")
                and len(profile.content.get("emails"))
                and profile.content.get("emails")[0]
            )
        ...
        profile_search_results = (
            self.openreview_client.search_profiles(
                confirmedEmails=email_members_list, ids=None, term=None
            )
            if email_members_list
            else {}
        )
        email_profiles = []
    ...
    def retrieve_expertise_helper(self, member, email):
        ...
        for n in member_papers:
            ...
            if (
                paper_title
                and n["id"] not in self.excluded_ids_by_user[member]
                and paper_title not in seen_keys
            ):
    ...
    def retrieve_expertise(self):
        ...
        valid_members, invalid_members = self.get_profile_ids(
            group_ids=group_ids, reviewer_ids=reviewer_ids
        )
        ...
        with ThreadPoolExecutor(max_workers=self.config.get("max_workers")) as executor:
            for (member, email) in valid_members:
                futures.append(
                    executor.submit(self.retrieve_expertise_helper, member, email)
                )
        self.pbar.close()
            ...
            with open(self.root.joinpath(csv_expertise)) as csv_file:
                ...
                for publication in tqdm(csv_reader):
                    ...
                    expertise[member_id].append(
                        {"id": _id, "content": {"title": title, "abstract": abstract}}
                    )
        ...
        if csv_submissions:
            ...
            with open(self.root.joinpath(csv_submissions)) as csv_file:
                ...
                for submission in tqdm(csv_reader):
                    ...
                    reduced_submissions[paper_id] = {
                        "id": paper_id,
                        "content": {"title": title, "abstract": abstract},
                    }
...
if __name__ == "__main__":
    ...
    client = openreview.Client(
        username=args.username, password=args.password, baseurl=args.baseurl
    )
