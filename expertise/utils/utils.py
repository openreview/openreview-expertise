...
def fixedwidth(item_list, list_len, pad_val=0):
    ...
    padded = np.pad(item_list, padding_bounds, "constant", constant_values=pad_val)
...
def get_bids_by_forum(dataset):
    ...
    pos_and_neg_signatures_by_forum = defaultdict(
    ...
    for forum_id, forum_bids in bids_by_forum.items():
        forum_bids_flat = [
            {"signature": bid["signature"], "bid": bid["tag"]} for bid in forum_bids
        ]
...
def format_data_bids(
    train_set_ids, bids_by_forum, kps_by_id, max_num_keyphrases=None, sequential=True
):
    """
    ...
    print(
        "WARNING: utils.format_data_bids is deprecated. Use dataset.bpr_bid_samples instead"
    )
    ...
    for forum_id in train_set_ids:
        if forum_id in submission_kps:
            ...
            pos_neg_pairs = itertools.product(
                forum_pos_signatures, forum_neg_signatures
            )
...
def matrix_to_ranklists(score_matrix):
    ...
    for paper_id in score_matrix:
        ...
        ranklist = [
            "{};{}".format(reviewer_score_tuple[0], reviewer_score_tuple[1])
            for reviewer_score_tuple in ranklist_w_scores
        ]
...
def extract_candidate_chunks(
    text, grammar=r"NP: {<JJ>*<NN>}", delimiter="_", stemmer=None
):
    ...
    for key, group in itertools.groupby(
        all_chunks, lambda word_pos_chunk_triple: word_pos_chunk_triple[2] != "O"
    ):
        if key:
            words = []
...
def extract_candidate_words(
    text, good_tags=set(["JJ", "JJR", "JJS", "NN", "NNP", "NNS", "NNPS"]), stemmer=None
):
    # exclude candidates that are stop words or entirely punctuation
    punct = set(string.punctuation)
    stop_words = set(nltk.corpus.stopwords.words("english"))
