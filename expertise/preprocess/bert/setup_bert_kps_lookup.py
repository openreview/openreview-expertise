...
def setup_bert_kps_lookup(config):
    ...
    archive_features_dir = os.path.join(
        config.experiment_dir, "setup", "archives-features"
    )
    submission_features_dir = os.path.join(
        config.experiment_dir, "setup", "submissions-features"
    )
        ...
        for filename in os.listdir(target_dir):
            ...
            for document in archives:
                ...
                for feature in features:
                    if (
                        feature["token"] in document_kps
                        and feature["token"] not in kps_seen
                    ):
