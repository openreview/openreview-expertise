...
def get_avg_words(line_features, layer_index=-1):
    ...
    for line_f in line_features:
        ...
        for token_feature in line_f["features"]:
            if not (
                token_feature["token"].startswith("[")
                and token_feature["token"].endswith("]")
            ):
...
def concatenate_layers(embeddings):
    ...
    for line in embeddings:
        ...
        for token_data in token_features:
            ...
            concatenated_vector = [
                value for layer in token_data["layers"] for value in layer["values"]
            ]
...
AGGREGATOR_MAP = {"avg": get_avg_words, "cls": get_cls_vectors, "all": get_all_vectors}
...
def get_embeddings(lines, pretrained_bert, **user_args):
    ...
    extraction_args = {"lines": lines, "model": model, "tokenizer": tokenizer}
