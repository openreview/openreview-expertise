...
def get_client(token):
    """Return an OpenReview v2 client for the provided token."""
    return openreview.api.OpenReviewClient(token=token, baseurl=current_app.config['OPENREVIEW_BASEURL_V2'])
...
def require_auth(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            or_client_v2 = get_client(token)
            user_id = get_user_id(or_client_v2)
