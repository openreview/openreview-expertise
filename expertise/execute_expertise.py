...
def execute_expertise(config):
    ...
    if config["model"] == "bm25":
        ...
        if not config["model_params"].get("skip_bm25", False):
            bm25Model.all_scores(
                preliminary_scores_path=Path(
                    config["model_params"]["scores_path"]
                ).joinpath("preliminary_scores.pkl"),
                scores_path=Path(config["model_params"]["scores_path"]).joinpath(
                    config["name"] + ".csv"
                ),
            )
    ...
    if config["model"] == "specter":
        ...
        specter_predictor = multifacet_recommender.SpecterPredictor(
            specter_dir=config["model_params"].get(
                "specter_dir", "./models/multifacet_recommender/specter/"
            ),
        ...
        if not config["model_params"].get("skip_specter", False):
            specter_predictor.embed_publications(
                publications_path=Path(
                    config["model_params"]["publications_path"]
                ).joinpath("pub2vec.jsonl")
            )
            specter_predictor.embed_submissions(
                submissions_path=Path(
                    config["model_params"]["submissions_path"]
                ).joinpath("sub2vec.jsonl")
            )
    ...
    if config["model"] == "mfr":
        ...
        mfr_predictor = multifacet_recommender.MultiFacetRecommender(
            work_dir=config["model_params"].get("work_dir", "./"),
            feature_vocab_file=config["model_params"].get(
                "feature_vocab_file", "./feature/dictionary_index"
            ),
            model_checkpoint_dir=config["model_params"].get(
    ...
    if config["model"] == "specter+mfr":
        ...
        ens_predictor = multifacet_recommender.EnsembleModel(
            ...
            mfr_feature_vocab_file=config["model_params"].get(
                "mfr_feature_vocab_file", "./feature/dictionary_index"
            ),
        ...
        ens_predictor.embed_publications(
            specter_publications_path=Path(
                config["model_params"]["publications_path"]
            ).joinpath("pub2vec.jsonl"),
